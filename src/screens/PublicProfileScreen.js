...
// Design Pixel Perfect + Sistema Seguir/Seguidores (Fase 2)
...
export default function PublicProfileScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        ...
        // Verificar se o usuário atual já segue este perfil
        if (currentUser && currentUser.uid !== userId) {
          verificarSeSegue(currentUser.uid, userId).then(setIsFollowing).catch(() => {});
        }

        // Carregar contagem de seguidores do profile
        setSeguidoresCount(dados.stats?.seguidores_count || 0);
  ...
  // === HANDLE SEGUIR (Optimistic UI) ===
  const handleSeguir = useCallback(async () => {
    if (!currentUser) {
      Alert.alert('Atenção', 'Faça login para seguir.');
      return;
    }
    if (currentUser.uid === userId) return;

    // Optimistic UI: inverte imediatamente
    const novoStatus = !isFollowing;
    setIsFollowing(novoStatus);
    setLoadingFollow(true);

    // Atualiza contagem localmente
    setSeguidoresCount((prev) => (novoStatus ? prev + 1 : Math.max(0, prev - 1)));

    try {
      await toggleSeguirUsuario(currentUser.uid, userId);
    } catch (error) {
      // Reverte em caso de erro
      setIsFollowing(!novoStatus);
      setSeguidoresCount((prev) => (novoStatus ? Math.max(0, prev - 1) : prev + 1));
  ...
  const tituloLabel =
    TITULOS_MINISTERIAIS.find((t) => t.value === profile?.titulo_ministerial)?.label || 'Membro';
  ...
  const conquistas = useMemo(() => {
    ...
    return [
      { icone: '🙏', label: '10 Pedidos', ativo: totalPedidos >= 10, descricao: `${totalPedidos} pedido(s)` },
      { icone: '🕊️', label: 'Testemunhos', ativo: totalTestemunhosFeitos >= 1, descricao: `${totalTestemunhosFeitos} testemunho(s)` },
      { icone: '💎', label: 'Apoiador', ativo: profile?.isPremium === true, descricao: profile?.isPremium ? 'Premium ativo' : 'Premium inativo' },
      { icone: '🔥', label: '30 Dias', ativo: diasMembro >= 30, descricao: `${diasMembro} dia(s)` },
      { icone: '👑', label: 'Reconhecido', ativo: isReconhecido, descricao: `${contagemReal} endosso(s)` },
      { icone: '🏠', label: 'Célula', ativo: celulas.length > 0, descricao: `${celulas.length} célula(s)` },
    ];
  }, [profile, testemunhos.length, celulas.length, isReconhecido, contagemReal]);

  const executarEndosso = useCallback(
    async (tipoEndosso) => {
      setEndossando(true);
      try {
  ...
  const handleEndossar = useCallback(async () => {
    ...
    if (ehLider) {
      Alert.alert('Opções de Endosso', 'Como líder deste membro, qual nível de endosso deseja conceder?', [
  ...
  const seguindoCount = statsData.seguindo_count || 0;
        ...
        <View style={styles.mainCard}>
          ...
          <View style={styles.statsHeaderSection}>
            ...
            <View style={styles.statsRow}>
              <View style={styles.statItem}>
                ...
                <Text style={styles.statItemText}>{seguidoresCount} Seguidores</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
                <Ionicons name="person-outline" size={16} color={COLORS.gray500} />
                <Text style={styles.statItemText}>{seguindoCount} Seguindo</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.acoesVibeRow}>
            <View style={styles.acoesColuna}>
              {/* Botão Seguir com Optimistic UI */}
              {currentUser && currentUser.uid !== userId ? (
                <TouchableOpacity
                  style={[styles.btnSeguir, isFollowing && styles.btnSeguindo]}
                  activeOpacity={0.85}
                  onPress={handleSeguir}
                  disabled={loadingFollow}
                >
                  {loadingFollow ? (
                    <ActivityIndicator size="small" color={isFollowing ? COLORS.gray600 : COLORS.white} />
                  ) : (
                    <Text style={[styles.btnSeguirText, isFollowing && styles.btnSeguindoText]}>
                  ...
                  >
                    {endossando ? (
                      ...
                      <>
                        ...
                        <Text style={styles.endossarBtnText}>{jaEndossou ? 'Remover Endosso' : 'Endossar Ministério'}</Text>
                      </>
                    )}
                  </TouchableOpacity>
                )}
...
const styles = StyleSheet.create({
  ...
  backgroundBlur: { position: 'absolute', top: 0, left: 0, right: 0, height: 180 },
  ...
  avatarSection: { alignItems: 'center', marginTop: -45 },
  fotoWrapper: { position: 'relative', alignSelf: 'center' },
  fotoPerfil: { width: 90, height: 90, borderRadius: 45, borderWidth: 3, borderColor: COLORS.white },
  avatarContainer: {
  ...
  seloVerificado: { position: 'absolute', bottom: 0, right: 0, backgroundColor: COLORS.white, borderRadius: 12 },
  nome: { fontSize: 22, fontWeight: '800', color: COLORS.gray800, textAlign: 'center', marginTop: SPACING.sm },
  tituloMinisterial: { fontSize: 14, color: COLORS.gray500, textAlign: 'center', marginTop: SPACING.xs },

  statsHeaderSection: { alignItems: 'center', marginTop: SPACING.xl, paddingHorizontal: SPACING.lg },
  statsSectionLabel: { fontSize: FONTS.sizes.xs, fontWeight: '700', color: COLORS.gray400, letterSpacing: 1.5, marginBottom: SPACING.sm },
  statsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
  statItem: { flexDirection: 'row', alignItems: 'center', gap: SPACING.xs },
  statItemText: { fontSize: FONTS.sizes.sm, color: COLORS.gray500, fontWeight: '500' },
  statDivider: { width: 1, height: 16, backgroundColor: COLORS.gray300, marginHorizontal: SPACING.md },

  bioSection: { marginHorizontal: SPACING.lg, marginTop: SPACING.lg },
  bioSectionLabel: { fontSize: FONTS.sizes.xs, fontWeight: '700', color: COLORS.gray400, letterSpacing: 1.5, marginBottom: SPACING.sm },
  bioText: { fontSize: FONTS.sizes.md, color: COLORS.gray700, lineHeight: 22, textAlign: 'left' },

  acoesVibeRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'stretch', marginTop: SPACING.lg, marginHorizontal: SPACING.lg },
  acoesColuna: { flex: 1, marginRight: SPACING.md, justifyContent: 'center' },

  btnSeguir: { backgroundColor: COLORS.primary, borderRadius: RADIUS.full, paddingVertical: 12, alignItems: 'center', justifyContent: 'center', marginBottom: SPACING.sm },
  btnSeguindo: { backgroundColor: 'transparent', borderWidth: 1.5, borderColor: COLORS.gray400 },
  btnSeguirText: { color: COLORS.white, fontSize: FONTS.sizes.sm, fontWeight: 'bold', letterSpacing: 0.5 },
  btnSeguindoText: { color: COLORS.gray600 },

  btnMensagem: { borderWidth: 1.5, borderColor: COLORS.primary, borderRadius: RADIUS.full, paddingVertical: 12, alignItems: 'center', justifyContent: 'center' },
  btnMensagemText: { color: COLORS.primary, fontSize: FONTS.sizes.sm, fontWeight: 'bold', letterSpacing: 0.5 },

  vibeCardQuadrado: { width: 110, borderRadius: 20, borderWidth: 1.5, borderColor: COLORS.primary + '40', backgroundColor: COLORS.primary + '05', alignItems: 'center', justifyContent: 'center', padding: SPACING.sm },
  vibeCardIconeGrande: { fontSize: 36, marginBottom: SPACING.xs },
  vibeCardLabelPequeno: { fontSize: FONTS.sizes.xs, fontWeight: '600', color: COLORS.gray600, textAlign: 'center' },

  interessesSection: { marginHorizontal: SPACING.lg, marginTop: SPACING.xl },
  interessesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: SPACING.sm },
  interessePill: { paddingHorizontal: SPACING.md, paddingVertical: SPACING.xs, borderRadius: RADIUS.full, borderWidth: 1.5, borderColor: COLORS.gray200, backgroundColor: 'transparent' },
  interessePillText: { fontSize: FONTS.sizes.sm, fontWeight: '600', color: COLORS.gray600 },
